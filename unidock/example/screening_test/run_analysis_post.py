...
log_file_path = "./result/def/2048/astex_log.txt"  # 定义日志文件的路径
    ...
    with open(csv_file, newline='') as csvfile:
        ...
        for row in reader:
            ...
            try:
                ...
                end_time = time.time()  # 记录命令结束执行的时间
                duration = end_time - start_time  # 计算持续时间
                result_message = f"命令执行成功: {command}\n执行时间: {duration} 秒\n"
            except subprocess.CalledProcessError as e:
                print(f"命令执行失败: {command}\n错误信息: {e}")
            print(result_message)
            with open(log_file_path, "a") as log_file:  # 以追加模式打开日志文件
                log_file.write(result_message)  # 将结果信息写入日志文件
            i+=1
    return commands

# 调用函数并打印结果
csv_file = '/home/yxyy/Documents/code/Uni-Dock/unidock/example/screening_test/astex_cleaned/pdb_center.csv'
