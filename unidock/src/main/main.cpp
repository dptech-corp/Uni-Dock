...
bug reporting, license agreements, and more information.      \n";
    ...
    try {
        ...
        advanced.add_options()("score_only", bool_switch(&score_only),
            ...
            "precalculated "
            "grids) for: (1) local optimization and scoring after docking, (2) --local_only jobs, "
            "and "
            "(3) --score_only jobs")("force_even_voxels", bool_switch(&force_even_voxels),
                                     "calculated grid maps will have an even number of voxels "
                                     "(intervals) in each dimension (odd number of grid points)")(
        ...
        misc.add_options()(
            "cpu", value<int>(&cpu)->default_value(0),
            "the number of CPUs to use (the default is to try to detect the number of "
            "CPUs or, failing that, use 1)")("seed", value<int>(&seed)->default_value(0),
            ...
            "spacing", value<double>(&grid_spacing)->default_value(0.375),
            "grid spacing (Angstrom)")("verbosity", value<int>(&verbosity)->default_value(1),
            ...
            "MC "
            "steps is based on heuristics)")("refine_step",
            ...
            "exhaustiveness and search steps; the higher the computational complexity, "
            "the higher the accuracy, but the larger the computational cost")
        ...
        if (vm.count("receptor") && vm.count("maps")) {
            std::cerr
                << "ERROR: Cannot specify both receptor and affinity maps at the same time, --flex "
                   "argument is allowed with receptor or maps.\n";
            exit(EXIT_FAILURE);
        }
