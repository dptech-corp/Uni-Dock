...
template<>
void template_batch_docking<SmallConfig>(Vina &v,std::vector<named_model> &all_ligands,std::vector<Ligand> &sized_group,
                        std::string batch_type,int exhaustiveness, bool multi_bias,float max_memory,
                        float receptor_atom_numbers,std::string out_dir,std::string bias_file,int num_modes,
                        double min_rmsd,int max_evals,int max_step,int seed, int refine_step,bool local_only, 
                        double energy_range){
    ...
    while (processed_ligands < sized_group.size()) {
        ...
        std::vector<model> batch_ligands;  // ligands in current batch
        v1.bias_batch_list.clear();
        while (predict_peak_memory<SmallConfig>(batch_size, exhaustiveness, all_atom2_numbers,
                                    multi_bias)
                    < max_memory
                && processed_ligands + batch_size < sized_group.size()) {
            batch_ligands.emplace_back(
...
template<>
void template_batch_docking<MediumConfig>(Vina &v,std::vector<named_model> &all_ligands,std::vector<Ligand> &sized_group,
                        std::string batch_type,int exhaustiveness, bool multi_bias,float max_memory,
                        float receptor_atom_numbers,std::string out_dir,std::string bias_file,int num_modes,
                        double min_rmsd,int max_evals,int max_step,int seed, int refine_step,bool local_only, 
                        double energy_range){
    ...
    while (processed_ligands < sized_group.size()) {
        ...
        std::vector<model> batch_ligands;  // ligands in current batch
        v1.bias_batch_list.clear();
        while (predict_peak_memory<MediumConfig>(batch_size, exhaustiveness, all_atom2_numbers,
                                    multi_bias)
                    < max_memory
                && processed_ligands + batch_size < sized_group.size()) {
            batch_ligands.emplace_back(
...
template<>
void template_batch_docking<LargeConfig>(Vina &v,std::vector<named_model> &all_ligands,std::vector<Ligand> &sized_group,
                        std::string batch_type,int exhaustiveness, bool multi_bias,float max_memory,
                        float receptor_atom_numbers,std::string out_dir,std::string bias_file,int num_modes,
                        double min_rmsd,int max_evals,int max_step,int seed, int refine_step,bool local_only, 
                        double energy_range){
    ...
    while (processed_ligands < sized_group.size()) {
        ...
        std::vector<model> batch_ligands;  // ligands in current batch
        v1.bias_batch_list.clear();
        while (predict_peak_memory<LargeConfig>(batch_size, exhaustiveness, all_atom2_numbers,
                                    multi_bias)
                    < max_memory
                && processed_ligands + batch_size < sized_group.size()) {
            batch_ligands.emplace_back(
...
template<>
void template_batch_docking<ExtraLargeConfig>(Vina &v,std::vector<named_model> &all_ligands,std::vector<Ligand> &sized_group,
                        std::string batch_type,int exhaustiveness, bool multi_bias,float max_memory,
                        float receptor_atom_numbers,std::string out_dir,std::string bias_file,int num_modes,
                        double min_rmsd,int max_evals,int max_step,int seed, int refine_step,bool local_only, 
                        double energy_range){
    ...
    while (processed_ligands < sized_group.size()) {
        ...
        std::vector<model> batch_ligands;  // ligands in current batch
        v1.bias_batch_list.clear();
        while (predict_peak_memory<ExtraLargeConfig>(batch_size, exhaustiveness, all_atom2_numbers,
                                    multi_bias)
                    < max_memory
                && processed_ligands + batch_size < sized_group.size()) {
            batch_ligands.emplace_back(
