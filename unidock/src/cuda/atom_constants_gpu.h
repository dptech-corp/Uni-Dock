...
__device__ __constant__ fl metal_covalent_radius_gpu = 1.75; // for metals not on the list // FIXME this info should be moved to non_ad_metals
...
__device__ __constant__ atom_equivalence_gpu atom_equivalence_data_gpu[] = {
	{"Se",  "S"}
};

__device__ __constant__ sz atom_equivalences_size_gpu = sizeof(atom_equivalence_data_gpu) / sizeof(atom_equivalence_gpu);

__device__ __constant__ acceptor_kind acceptor_kind_data_gpu[] = { // ad_type, optimal length, depth
	{AD_TYPE_NA, 1.9, 5.0},
	{AD_TYPE_OA, 1.9, 5.0},
	{AD_TYPE_SA, 2.5, 1.0}
};

__device__ __constant__ sz acceptor_kinds_size_gpu = sizeof(acceptor_kind_data_gpu) / sizeof(acceptor_kind);
...
__device__ __forceinline__  bool is_non_ad_metal_name_gpu(const char name[4]) {
	...
	VINA_FOR(i, s)
		if(strcmp(non_ad_metal_names_gpu[i], name) == 0)
			return true;
	return false;
}
...
__device__ __forceinline__  sz string_to_ad_type_gpu(const char *name) { // returns AD_TYPE_SIZE if not found (no exceptions thrown, because metals unknown to AD4 are not exceptional)
    VINA_FOR(i, atom_kinds_size_gpu)
		if(strcmp(atom_kind_data_gpu[i].name, name) == 0)
			return i;
