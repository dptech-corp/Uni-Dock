...
__device__ __forceinline__ void quaternion_to_r3(const float* q, float* orientation_m) {
    ...
    matrix_d tmp;
    mat_init(&tmp, 0); /* matrix_d with fixed dimension 3(here we treate this as a regular
                          matrix_d(not triangular matrix_d!)) */
...
__device__ __forceinline__ void p_eval_deriv(float* out, int type_pair_index, float r2,
    ...
    if (r2 > cutoff_sqr)
        DEBUG_PRINTF("\nkernel2: p_eval_deriv() ERROR!, r2 > Cutoff_sqr, r2=%f, cutoff_sqr=%f", r2,
                     cutoff_sqr);
...
__device__ __forceinline__ void POT_deriv(const m_minus_forces_t* minus_forces,
    ...
    float torsion_derivative[MAX_NUM_OF_RIGID]; /* torsion_derivative[0] has no meaning(root node
                                                   has no torsion) */
    ...
    for (int i = 0; i < num_rigid; i++) {
        ...
        for (int j = begin; j < end; j++) {
            ...
            float tmp2[3] = {minus_forces->coords[j][0], minus_forces->coords[j][1],
...
__device__ __forceinline__ float m_eval_deriv(output_type_cuda_t* c, change_cuda_t* g,
...
__device__ __forceinline__ float line_search(m_cuda_t* m_cuda_gpu, p_cuda_t* p_cuda_gpu,
...
__device__ __forceinline__ void bfgs(output_type_cuda_t* x, change_cuda_t* g, m_cuda_t* m_cuda_gpu,
                                     p_cuda_t* p_cuda_gpu, ig_cuda_t* ig_cuda_gpu,
                                     const float* hunt_cap, const float epsilon_fl,
                                     const int max_steps) {
...
__device__ __forceinline__ void bfgs_only_torsion(output_type_cuda_t* x, change_cuda_t* g, m_cuda_t* m_cuda_gpu,
                                     p_cuda_t* p_cuda_gpu, ig_cuda_t* ig_cuda_gpu,
                                     const float* hunt_cap, const float epsilon_fl,
                                     const int max_steps,const float mobility) {
    int n = 3 + 3 + x->lig_torsion_size; /* the dimensions of matirx */
    ...
    for (int step = 0; step < max_steps; step++) {
        ...
        output_type_cuda_init_with_output(x, &x_new);
        if (!(sqrt(scalar_product(g, g, n)) >= 1e-5)) break;
        change_cuda_init_with_torsion_change(g, &g_new,mobility);
...
__device__ __forceinline__ void m_cuda_init_with_m_cuda(const m_cuda_t* m_cuda_old,
                                                        m_cuda_t* m_cuda_new) {
...
__global__ __launch_bounds__(MAX_THREADS_PER_BLOCK, MIN_BLOCKS_PER_MP) void kernel(
    ...
    float* hunt_cap_gpu, float* authentic_v_gpu, output_type_cuda_t* results, int search_depth,
    int num_of_ligands, int threads_per_ligand, bool multi_bias) {
    ...
    if (idx < num_of_ligands * threads_per_ligand) {
        ...
        output_type_cuda_init(&tmp,
                              rand_molec_struc_gpu + idx * (SIZE_OF_MOLEC_STRUC / sizeof(float)));
        ...
        for (int step = 0; step < search_depth; step++) {
            ...
            bfgs(&candidate, &g, &m_cuda_gpu, p_cuda_gpu, ig_cuda_gpu, hunt_cap_gpu, epsilon_fl,
                 bfgs_max_steps);
            // n ~ U[0,1]
            ...
            if (step == 0 || metropolis_accept(tmp.e, candidate.e, 1.2, n)) {
                ...
                set(&tmp, &m_cuda_gpu.ligand.rigid, &m_cuda_gpu.m_coords, m_cuda_gpu.atoms,
                    m_cuda_gpu.m_num_movable_atoms, epsilon_fl);
                if (tmp.e < best_e) {
                    bfgs(&tmp, &g, &m_cuda_gpu, p_cuda_gpu, ig_cuda_gpu, authentic_v_gpu,
...
#define MAX_THREADS_PER_BLOCK 32
#define MIN_BLOCKS_PER_MP 32
__global__ __launch_bounds__(MAX_THREADS_PER_BLOCK, MIN_BLOCKS_PER_MP) void kernel_torsion(
    m_cuda_t* m_cuda_global, ig_cuda_t* ig_cuda_gpu, p_cuda_t* p_cuda_gpu,
    float* rand_molec_struc_gpu, float* best_e_gpu, int bfgs_max_steps, float mutation_amplitude,
    curandStatePhilox4_32_10_t* states, unsigned long long seed, float epsilon_fl,
    float* hunt_cap_gpu, float* authentic_v_gpu, output_type_cuda_t* results, int search_depth,
    int num_of_ligands, int threads_per_ligand, bool multi_bias,float mobility=0.01) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    float best_e = INFINITY;
...
__host__ void monte_carlo::operator()(
    ...
    kernel<<<thread / 32 + 1, 32>>>(m_cuda_gpu, ig_cuda_gpu, p_cuda_gpu, rand_molec_struc_gpu,
                                    ...
                                    mutation_amplitude_float, states, seed, epsilon_fl_float,
                                    hunt_cap_gpu, authentic_v_gpu, results_gpu, global_steps,
...
__host__ void monte_carlo::operator()(
    ...
    std::vector<precalculate_byatom>& p_gpu, triangular_matrix_cuda_t* m_data_list_gpu,
    const igrid& ig, const vec& corner1, const vec& corner2, rng& generator, int verbosity,
    unsigned long long seed, std::vector<std::vector<bias_element>>& bias_batch_list,bool core_constraint=false,float mobility=0.01) const {
    ...
    printf("launch kernel, global_steps=%d, thread=%d, num_of_ligands=%d\n", global_steps,
                 thread, num_of_ligands);
    if (core_constraint){
        printf("core constraint mode\n");
        kernel_torsion<<<thread / 32 + 1, 32>>>(m_cuda_gpu, ig_cuda_gpu, p_cuda_gpu, rand_molec_struc_gpu,
                                    best_e_gpu, quasi_newton_par_max_steps,
                                    mutation_amplitude_float, states, seed, epsilon_fl_float,
                                    hunt_cap_gpu, authentic_v_gpu, results_gpu, global_steps,
                                    num_of_ligands, threads_per_ligand, multi_bias,mobility);
