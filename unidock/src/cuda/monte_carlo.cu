...
__device__ __forceinline__ void output_type_cuda_init_with_output(output_type_cuda_t *out_new, const output_type_cuda_t *out_old)
{
...
__device__ __forceinline__ void output_type_cuda_increment(output_type_cuda_t *x, const change_cuda_t *c, float factor, float epsilon_fl)
{
	// position increment
	for (int k = 0; k < 3; k++)
...
__device__ __forceinline__ void random_inside_sphere_gpu(float *random_inside_sphere, curandStatePhilox4_32_10_t *state)
{
...
__device__ __forceinline__ void angle_to_quaternion(float *q, const float *rotation, float epsilon_fl)
{
...
__device__ __forceinline__ float gyration_radius(int m_lig_begin,
												 int m_lig_end,
												 ...
												 const float *m_lig_node_origin)
{
	...
	for (int i = m_lig_begin; i < m_lig_end; i++)
	{
		...
		if (atoms[i].types[0] != EL_TYPE_H)
		{ // for el, we use the first element (atoms[i].types[0])
												 ...
												 const float amplitude)
{
	...
	if (which == 0)
	{
		...
		if (gr > epsilon_fl)
		{
			float rotation[3];
			for (int i = 0; i < 3; i++)
				rotation[i] = amplitude / gr * random_inside_sphere[i];
...
__device__ __forceinline__ void mat_init(matrix_d *m, float fill_data)
{
	...
	if (9 > MAX_HESSIAN_MATRIX_D_SIZE)
...
// as regular matrix_d
__device__ __forceinline__ void matrix_d_set_element(matrix_d *m, int dim, int x, int y, float fill_data)
{
	m->data[x + y * dim] = fill_data;
}

__device__ __forceinline__ void matrix_d_set_element_tri(matrix_d *m, int x, int y, float fill_data)
{
...
__device__ __forceinline__ void change_cuda_init(change_cuda_t *g, const float *ptr)
{
	for (int i = 0; i < 3; i++)
		g->position[i] = ptr[i];
	for (int i = 0; i < 3; i++)
		g->orientation[i] = ptr[i + 3];
	for (int i = 0; i < MAX_NUM_OF_LIG_TORSION; i++)
		g->lig_torsion[i] = ptr[i + 3 + 3];
...
__device__ __forceinline__ void change_cuda_init_with_change(change_cuda_t *g_new, const change_cuda_t *g_old)
{
	for (int i = 0; i < 3; i++)
		g_new->position[i] = g_old->position[i];
	for (int i = 0; i < 3; i++)
		g_new->orientation[i] = g_old->orientation[i];
	for (int i = 0; i < MAX_NUM_OF_LIG_TORSION; i++)
		g_new->lig_torsion[i] = g_old->lig_torsion[i];
	for (int i = 0; i < MAX_NUM_OF_FLEX_TORSION; i++)
...
__device__ __forceinline__ float access_m_data(float *m_data, int m_i, int m_j, int i, int j, int k)
{
...
__device__ __forceinline__ void curl_with_deriv(float *e, float *deriv, float v, const float epsilon_fl)
{
...
__device__ __forceinline__ float g_evaluate(grid_cuda_t *g,
...
__device__ __forceinline__ float ig_eval_deriv(output_type_cuda_t *x,
											   change_cuda_t *g,
											   const float v,
											   ...
											   const float epsilon_fl)
{
	...
	for (int i = 0; i < m_cuda_gpu->m_num_movable_atoms; i++)
	{
		...
		{
			for (int j = 0; j < 3; j++)
				m_cuda_gpu->minus_forces.coords[i][j] = 0;
			continue;
		}
		float deriv[3];
		...
		for (int j = 0; j < 3; j++)
			m_cuda_gpu->minus_forces.coords[i][j] = deriv[j];
	}
	return e;
}
...
__device__ __forceinline__ void quaternion_to_r3(const float *q, float *orientation_m)
{
	...
	for (int i = 0; i < 9; i++)
		orientation_m[i] = tmp.data[i];
}

__device__ __forceinline__ void local_to_lab_direction(float *out,
...
__device__ __forceinline__ void local_to_lab(float *out,
											 const float *origin,
...
__device__ __forceinline__ void angle_to_quaternion2(float *out,
													 const float *axis,
													 float angle)
{
	normalize_angle(&angle);
...
__device__ __forceinline__ void set(const output_type_cuda_t *x,
									...
									const float epsilon_fl)
{
	...
	for (int i = 0; i < 3; i++)
		lig_rigid_gpu->origin[0][i] = x->position[i];
	for (int i = 0; i < 4; i++)
	...
	for (int current = 1; current < lig_rigid_gpu->num_children + 1; current++)
	{ /* current starts from 1 (namely starts from first child node) */
...
__device__ __forceinline__ void p_eval_deriv(float *out,
											 int type_pair_index,
											 float r2,
											 p_cuda_t *p_cuda_gpu,
											 const float epsilon_fl)
...
__device__ __forceinline__ void curl(float *e, float *deriv, float v, const float epsilon_fl)
{
	if (*e > 0 && not_max_gpu(v))
	{
		...
		(*e) = tmp * (*e);
		for (int i = 0; i < 3; i++)
			deriv[i] = deriv[i] * (tmp * tmp);
	}
}

__device__ __forceinline__ float eval_interacting_pairs_deriv(p_cuda_t *p_cuda_gpu,
															  const float v,
...
__device__ __forceinline__ void product(float *res, const float *a, const float *b)
{
...
__device__ __forceinline__ void POT_deriv(const m_minus_forces_t *minus_forces,
										  ...
										  change_cuda_t *g)
{
	...
	for (int i = 0; i < num_rigid; i++)
	{
		...
		{
			for (int k = 0; k < 3; k++)
				position_derivative_tmp[i][k] += minus_forces->coords[j][k];
	...
	{ /* from bottom to top */
		for (int k = 0; k < 3; k++)
			position_derivative[i][k] = position_derivative_tmp[i][k];
		...
		for (int j = 0; j < num_rigid; j++)
		{
			...
			{
				for (int k = 0; k < 3; k++)
					position_derivative[i][k] += position_derivative[j][k]; /* self+children node */
			}
		}
	}
	...
	{ /* from bottom to top */
		...
		for (int j = 0; j < num_rigid; j++)
		{
			...
			{ /* self + children node + product */
				...
				for (int k = 0; k < 3; k++)
					orientation_derivative[i][k] += product_out[k];
			}
		}
	}
	...
	for (int i = num_rigid - 1; i >= 0; i--)
	{
		float sum = 0;
		for (int j = 0; j < 3; j++)
			sum += orientation_derivative[i][j] * lig_rigid_gpu->axis[i][j];
	...
	for (int k = 0; k < num_torsion; k++)
		g->lig_torsion[k] = torsion_derivative[k + 1];
}

__device__ __forceinline__ float m_eval_deriv(output_type_cuda_t *c,
											  change_cuda_t *g,
											  m_cuda_t *m_cuda_gpu,
											  p_cuda_t *p_cuda_gpu,
											  ig_cuda_t *ig_cuda_gpu,
...
{
	if (index < 3)
		return g->position[index];
	index -= 3;
	if (index < 3)
		return g->orientation[index];
	index -= 3;
	if (index < g->lig_torsion_size)
		return g->lig_torsion[index];
...
__device__ __forceinline__ void minus_mat_vec_product(const matrix_d *h,
...
__device__ __forceinline__ float scalar_product(const change_cuda_t *a,
...
__device__ __forceinline__ float line_search(m_cuda_t *m_cuda_gpu,
											 p_cuda_t *p_cuda_gpu,
											 ig_cuda_t *ig_cuda_gpu,
											 int n,
...
__device__ __forceinline__ bool bfgs_update(matrix_d *h,
											const change_cuda_t *p,
											...
											const float epsilon_fl)
{
	...
	for (int i = 0; i < n; i++)
	{
		for (int j = i; j < n; j++)
		{
			float tmp = alpha * r * (find_change_index_read(&minus_hy, i) * find_change_index_read(p, j) + find_change_index_read(&minus_hy, j) * find_change_index_read(p, i)) +
...
__device__ __forceinline__ void bfgs(output_type_cuda_t *x,
									 change_cuda_t *g,
									 m_cuda_t *m_cuda_gpu,
									 p_cuda_t *p_cuda_gpu,
									 ig_cuda_t *ig_cuda_gpu,
									 ...
									 const int max_steps)
{
	...
	for (int step = 0; step < max_steps; step++)
	{
		...
		if (!(sqrt(scalar_product(g, g, n)) >= 1e-5))
			break;
...
__device__ __forceinline__ void m_cuda_init_with_m_cuda(const m_cuda_t *m_cuda_old, m_cuda_t *m_cuda_new)
{
...
__device__ __forceinline__ void get_heavy_atom_movable_coords(output_type_cuda_t *tmp, const m_cuda_t *m_cuda_gpu)
{
	int counter = 0;
	for (int i = 0; i < m_cuda_gpu->m_num_movable_atoms; i++)
	{
		...
		{
			for (int j = 0; j < 3; j++)
				tmp->coords[counter][j] = m_cuda_gpu->m_coords.coords[i][j];
			counter++;
		}
		else
		{
...
__device__ __forceinline__ bool metropolis_accept(float old_f, float new_f, float temperature, float n)
{
	if (new_f < old_f)
		return true;
...
__device__ __forceinline__ void write_back(output_type_cuda_t *results, const output_type_cuda_t *best_out)
{
	for (int i = 0; i < 3; i++)
		results->position[i] = best_out->position[i];
	for (int i = 0; i < 4; i++)
		results->orientation[i] = best_out->orientation[i];
	for (int i = 0; i < MAX_NUM_OF_LIG_TORSION; i++)
...
__global__
__launch_bounds__(MAX_THREADS_PER_BLOCK, MIN_BLOCKS_PER_MP) void kernel(m_cuda_t *m_cuda_global,
																		ig_cuda_t *ig_cuda_gpu,
																		p_cuda_t *p_cuda_gpu,
...
std::vector<output_type> monte_carlo::cuda_to_vina(output_type_cuda_t results_ptr[], int thread) const
{
	...
	for (int i = 0; i < thread; ++i)
	{
		...
		// Position
		for (int j = 0; j < 3; j++)
			tmp_c.ligands[0].rigid.position[j] = results.position[j];
		// Orientation
...
__host__ void monte_carlo::operator()(std::vector<model> &m_gpu, std::vector<output_container> &out_gpu, std::vector<precalculate_byatom> &p_gpu,
									  ...
									  int verbosity, unsigned long long seed, std::vector<std::vector<bias_element>> &bias_batch_list) const
{
	...
	for (int l = 0; l < num_of_ligands; ++l)
	{
		...
		else
		{
			...
			for (int i = 0; i < 9; i++)
				m_cuda->ligand.rigid.orientation_m[0][i] = m_ligand.node.get_orientation_m().data[i];
			...
			for (int i = 0; i < threads_per_ligand; ++i)
			{
				...
				for (int j = 0; j < lig_torsion_size; j++)
					rand_molec_struc_tmp->lig_torsion[j] = tmp.c.ligands[0].torsions[j]; // Only support one ligand
				assert(flex_torsion_size <= MAX_NUM_OF_FLEX_TORSION);
				for (int j = 0; j < flex_torsion_size; j++)
					rand_molec_struc_tmp->flex_torsion[j] = tmp.c.flex[0].torsions[j]; // Only support one flex
...
__host__ void monte_carlo::operator()(model &m, output_container &out, const precalculate_byatom &p, const igrid &ig, const vec &corner1, const vec &corner2, rng &generator) const
{
	int evalcount = 0;
	vec authentic_v(1000, 1000, 1000); // FIXME? this is here to avoid max_fl/max_fl
	conf_size s = m.get_size();
