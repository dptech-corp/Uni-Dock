...
models parse_multimodel_pdbqt(const std::string& input) {
    ...
    while (std::getline(in, str)) {
        ...
        if (starts_with(str, "MODEL")) {
            if (parsing_model == true || parsing_ligand == false)
                throw struct_parse_error("Misplaced MODEL tag at line " + std::to_string(count)
                                         + ".");
            tmp.push_back(model());
            parsing_model = true;
        } else if (starts_with(str, "ENDMDL")) {
            if (parsing_model == false || parsing_ligand == false)
                throw struct_parse_error("Misplaced ENDMDL tag at line " + std::to_string(count)
                                         + ".");
            parsing_model = false;
        } else if (starts_with(str, "BEGIN_RES")) {
            if (parsing_model == false || parsing_ligand == false)
                throw struct_parse_error("Misplaced BEGIN_RES tag at line " + std::to_string(count)
                                         + ".");
            parsing_ligand = false;
            tmp.back().flex.push_back(str);
        } else if (starts_with(str, "END_RES")) {
            if (parsing_model == false || parsing_ligand == true)
                throw struct_parse_error("Misplaced END_RES tag at line " + std::to_string(count)
                                         + ".");
            parsing_ligand = true;
            tmp.back().flex.push_back(str);
        } else {
            if (parsing_model == false)
                throw struct_parse_error("Input occurs outside MODEL at line "
                                         + std::to_string(count) + ".");
            if (parsing_ligand) {
    ...
    if (parsing_model == true)
        throw struct_parse_error("Missing ENDMDL tag at line " + std::to_string(count + 1) + ".");
    return tmp;
}
...
void write_pdbqt(const strl& lines, const std::string& name) {
    if (lines.size() > 0) {
        ...
        for (strl::const_iterator it = lines.begin(); it != lines.end(); ++it) out << *it << '\n';
    }
}
...
Thank you!\n";
    ...
    try {
        ...
        outputs.add_options()("ligand", value<std::string>(&ligand_prefix), "prefix for ligands")(
