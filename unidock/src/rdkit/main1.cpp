...
int main() {
    // Example usage
    std::string smiles = "CC(C)CC1=CC=CC=C1";
    ROMol* mol = SmilesToMol(smiles);
    if (!mol) {
        std::cerr << "Error: Failed to parse SMILES." << std::endl;
        return 1;
    }
    ...
    SDWriter original_writer("original_molecule.sdf");
    original_writer.write(*mol);
    original_writer.close();

    std::vector<int> torsion_atom_idx_list = {0, 1, 2, 3};
    int root_atom_idx = 0;
    ...
    SDWriter modified_writer("modified_molecule.sdf");
