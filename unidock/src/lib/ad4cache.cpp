...
float ad4cache::get_slope() const { return this->m_slope; }

// Attention: new gridding method
std::vector<grid> ad4cache::get_grids() const { return this->m_grids; }
...
void ad4cache::set_bias(const std::vector<bias_element> bias_list) {
    for (auto bias = bias_list.begin(); bias != bias_list.end(); ++bias) {
        DEBUG_PRINTF("bias info: type=%d, x=%f y=%f z=%f, Vset=%f, r=%f\n", bias->type,
                     bias->coords[0], bias->coords[1], bias->coords[2], bias->vset, bias->r);
        switch (bias->type) {
...
bool ad4cache::are_atom_types_grid_initialized(szv atom_types) const {
    VINA_FOR_IN(i, atom_types) {
        ...
        if (!is_atom_type_grid_initialized(t)) {
            std::cerr << "ERROR: Affinity map for atom type " << get_adtype_str(t)
                      << " is not present.\n";
            return false;
        }
    }
...
void ad4cache::write(const std::string& out_prefix, const szv& atom_types,
    ...
    VINA_FOR_IN(i, atom_types) {
        ...
        if (t == AD_TYPE_C && got_C_already) continue;
        if (t == AD_TYPE_C) got_C_already = true;
        ...
        if (m_grids[t].initialized()) {
            ...
            if ((m_grids[t].m_factor_inv[0] != m_grids[t].m_factor_inv[1])
                & (m_grids[t].m_factor_inv[0] != m_grids[t].m_factor_inv[2])) {
                printf("m_factor_inv x=%f, y=%f, z=%f\n", m_grids[t].m_factor_inv[0],
                       m_grids[t].m_factor_inv[1], m_grids[t].m_factor_inv[2]);
                return;
            }

            out << "SPACING " << m_grids[t].m_factor_inv[0] << "\n";

            // The number of elements in the grid is an odd number. But NELEMENTS has to be an even
            // number.
            int size_x = (m_grids[t].m_data.dim0() % 2 == 0) ? m_grids[t].m_data.dim0()
                                                             : m_grids[t].m_data.dim0() - 1;
            int size_y = (m_grids[t].m_data.dim1() % 2 == 0) ? m_grids[t].m_data.dim1()
                                                             : m_grids[t].m_data.dim1() - 1;
            int size_z = (m_grids[t].m_data.dim2() % 2 == 0) ? m_grids[t].m_data.dim2()
                                                             : m_grids[t].m_data.dim2() - 1;
