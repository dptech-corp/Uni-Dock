...
template <typename F, bool Sync = false> struct parallel_for : private boost::thread_group {
    ...
    void run(sz size_) {
        ...
        while (count_finished
               < num_threads)  // wait until processing of all elements is thread_finished
            busy.wait(self_lk);
    }
...
template <typename F> struct parallel_for<F, true> : private boost::thread_group {
    parallel_for(const F* f, sz num_threads)
...
template <typename F, typename Container, typename Input, bool Sync = false> struct parallel_iter {
