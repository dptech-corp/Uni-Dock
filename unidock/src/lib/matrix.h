...
public:
    ...
    void resize(sz m, sz n,
                const T& filler_val) {  // new sizes should be the same or greater than the old
...
template <typename T> class triangular_matrix {
public:
    sz m_dim;
...
template <typename T> class strictly_triangular_matrix {
    std::vector<T> m_data;
    sz m_dim;
