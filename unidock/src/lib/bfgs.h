...
template <typename Change> inline fl scalar_product(const Change& a, const Change& b, sz n) {
    fl tmp = 0;
    VINA_FOR(i, n)
    tmp += a(i) * b(i);
    return tmp;
...
inline bool bfgs_update(flmat& h, const Change& p, const Change& y, const fl alpha) {
    ...
    h(i, j)
        += alpha * r * (minus_hy(i) * p(j) + minus_hy(j) * p(i))
           + +alpha * alpha * (r * r * yhy + r) * p(i) * p(j);  // s * s == alpha * alpha * p * p
    return true;
}

template <typename F, typename Conf, typename Change>
fl line_search(F& f, sz n, const Conf& x, const Change& g, const fl f0, const Change& p,
               Conf& x_new, Change& g_new, fl& f1, int& evalcount) {  // returns alpha
    const fl c0 = 0.0001;
...
template <typename Change> void subtract_change(Change& b, const Change& a, sz n) {  // b -= a
    VINA_FOR(i, n)
    b(i) -= a(i);
}

template <typename F, typename Conf, typename Change>
fl bfgs(F& f, Conf& x, Change& g, const unsigned max_steps, const fl average_required_improvement,
        const sz over,
    ...
    VINA_U_FOR(step, max_steps) {
        ...
        if (step == 0) {
            ...
            if (std::abs(yy) > epsilon_fl) set_diagonal(h, alpha * scalar_product(y, p, n) / yy);
        }
