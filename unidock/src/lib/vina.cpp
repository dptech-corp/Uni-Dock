...
void Vina::global_search_gpu_small(const int exhaustiveness, const int n_poses, const double min_rmsd,
                             const int max_evals, const int max_step, int num_of_ligands,
                             unsigned long long seed, const int refine_step,
                             const bool local_only) {
    // Vina search (Monte-carlo and local optimization)
    ...
    if (m_sf_choice == SF_VINA || m_sf_choice == SF_VINARDO) {
        mc.do_docking<SmallConfig>(m_model_gpu, poses_gpu, m_precalculated_byatom_gpu, m_data_list_gpu, m_grid,
           m_grid.corner1(), m_grid.corner2(), generator, m_verbosity, seed, bias_batch_list);
    } else {
        mc.do_docking<SmallConfig>(m_model_gpu, poses_gpu, m_precalculated_byatom_gpu, m_data_list_gpu, m_ad4grid,
           m_ad4grid.corner1(), m_ad4grid.corner2(), generator, m_verbosity, seed, bias_batch_list);
    }
    auto end = std::chrono::system_clock::now();
    std::cout << "Kernel running time: "
...
void Vina::global_search_gpu_medium(const int exhaustiveness, const int n_poses, const double min_rmsd,
                             const int max_evals, const int max_step, int num_of_ligands,
                             unsigned long long seed, const int refine_step,
                             const bool local_only) {
    // Vina search (Monte-carlo and local optimization)
    ...
    if (m_sf_choice == SF_VINA || m_sf_choice == SF_VINARDO) {
        mc.do_docking<MediumConfig>(m_model_gpu, poses_gpu, m_precalculated_byatom_gpu, m_data_list_gpu, m_grid,
           m_grid.corner1(), m_grid.corner2(), generator, m_verbosity, seed, bias_batch_list);
    } else {
        mc.do_docking<MediumConfig>(m_model_gpu, poses_gpu, m_precalculated_byatom_gpu, m_data_list_gpu, m_ad4grid,
           m_ad4grid.corner1(), m_ad4grid.corner2(), generator, m_verbosity, seed, bias_batch_list);
    }
    auto end = std::chrono::system_clock::now();
    std::cout << "Kernel running time: "
...
void Vina::global_search_gpu_large(const int exhaustiveness, const int n_poses, const double min_rmsd,
                             const int max_evals, const int max_step, int num_of_ligands,
                             unsigned long long seed, const int refine_step,
                             const bool local_only) {
    // Vina search (Monte-carlo and local optimization)
    ...
    if (m_sf_choice == SF_VINA || m_sf_choice == SF_VINARDO) {
        mc.do_docking<LargeConfig>(m_model_gpu, poses_gpu, m_precalculated_byatom_gpu, m_data_list_gpu, m_grid,
           m_grid.corner1(), m_grid.corner2(), generator, m_verbosity, seed, bias_batch_list);
    } else {
        mc.do_docking<LargeConfig>(m_model_gpu, poses_gpu, m_precalculated_byatom_gpu, m_data_list_gpu, m_ad4grid,
           m_ad4grid.corner1(), m_ad4grid.corner2(), generator, m_verbosity, seed, bias_batch_list);
    }
    auto end = std::chrono::system_clock::now();
    std::cout << "Kernel running time: "
...
void Vina::global_search_gpu_extra_large(const int exhaustiveness, const int n_poses, const double min_rmsd,
                             const int max_evals, const int max_step, int num_of_ligands,
                             unsigned long long seed, const int refine_step,
                             const bool local_only) {
    // Vina search (Monte-carlo and local optimization)
    ...
    if (m_sf_choice == SF_VINA || m_sf_choice == SF_VINARDO) {
        mc.do_docking<ExtraLargeConfig>(m_model_gpu, poses_gpu, m_precalculated_byatom_gpu, m_data_list_gpu, m_grid,
           m_grid.corner1(), m_grid.corner2(), generator, m_verbosity, seed, bias_batch_list);
    } else {
        mc.do_docking<ExtraLargeConfig>(m_model_gpu, poses_gpu, m_precalculated_byatom_gpu, m_data_list_gpu, m_ad4grid,
           m_ad4grid.corner1(), m_ad4grid.corner2(), generator, m_verbosity, seed, bias_batch_list);
    }
    auto end = std::chrono::system_clock::now();
    std::cout << "Kernel running time: "
