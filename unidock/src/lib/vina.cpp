...
void Vina::set_receptor(const std::string& rigid_name, const std::string& flex_name) {
    ...
    */
    if (rigid_name.empty() && flex_name.empty() && m_sf_choice == SF_VINA) {
        ...
        // CONDITIONS 2, 3
        std::cerr
            << "ERROR: Only flexible residues allowed with the AD4 scoring function. No (rigid) "
               "receptor.\n";
        exit(EXIT_FAILURE);
    }
...
void Vina::compute_vina_maps(double center_x, double center_y, double center_z, double size_x,
    ...
    if (m_sf_choice == SF_AD42) {
        ...
        // m_model
        std::cerr
            << "ERROR: Cannot compute Vina or Vinardo affinity maps. The (rigid) receptor was "
               "not initialized.\n";
        exit(EXIT_FAILURE);
...
std::vector<double> Vina::score() {
    ...
    if (!m_ligand_initialized) {
        ...
        std::cerr
            << "ERROR: The ligand is outside the grid box. Increase the size of the grid box or "
               "center it accordingly around the ligand.\n";
        exit(EXIT_FAILURE);
    }
...
void Vina::global_search(const int exhaustiveness, const int n_poses, const double min_rmsd,
    ...
    if (!poses.empty()) {
        ...
        std::cerr
            << "WARNING: Check that it is large enough for all movable atoms, including those in "
               "the flexible side chains.\n";
    }
    ...
    for (int l = 0; l < num_of_ligands; ++l) {
        ...
        if (!poses.empty()) {
            ...
            std::cerr << "WARNING: Could not find any conformations completely within the search "
                         "space.\n";
            std::cerr
                << "WARNING: Check that it is large enough for all movable atoms, including those "
                   "in the flexible side chains.\n";
