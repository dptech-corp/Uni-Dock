...
bool cache::are_atom_types_grid_initialized(szv atom_types) const {
    ...
    VINA_FOR_IN(i, atom_types) {
        ...
        if (!is_atom_type_grid_initialized(t)) {
            std::cerr << "ERROR: Affinity map for atom type " << convert_XS_to_string(t)
                      << " is not present.\n";
            return false;
        }
    }
...
void cache::write(const std::string& out_prefix, const szv& atom_types,
                  const std::string& gpf_filename, const std::string& fld_filename,
                  const std::string& receptor_filename) {
    sz nat = num_atom_types(atom_type::XS);
    ...
    VINA_FOR_IN(i, atom_types) {
        ...
        if (m_grids[t].initialized()) {
            ...
            // For writing a .map, the number of points in the grid must be odd, which means that
            // the number of voxels (NELEMENTS) must be even... n_voxels = n_grid_points - 1
            if (nx % 2 == 1 || ny % 2 == 1 || nz % 2 == 1) {
                std::cerr << "ERROR: Can't write maps. Number of voxels (NELEMENTS) is odd. Use "
                             "--force_even_voxels.\n";
                exit(EXIT_FAILURE);
            } else {
...
std::vector<grid> cache::get_grids() const { return this->m_grids; }

int cache::get_atu() const { return atom_type::XS; }

void cache::populate_no_bias(const model& m, const precalculate& p, const szv& atom_types_needed) {
    szv needed;
    bool got_C_H_already = false;
    bool got_C_P_already = false;
                ...
                VINA_FOR_IN(possibilities_i, possibilities) {
                    ...
                    if (r2 <= cutoff_sqr) {
                        VINA_FOR_IN(j, needed) {
                            ...
                            assert(t2 < nat);
                            const sz type_pair_index
                                = triangular_matrix_index_permissive(nat, t1, t2);
                ...
                for (auto bias = bias_list.begin(); bias != bias_list.end(); ++bias) {
                    ...
                    switch (bias->type) {
                        ...
                        case bias_element::itype::acc: {  // OA, NA
                            ...
                            if (m_grids[XS_TYPE_O_A].initialized())
                                m_grids[XS_TYPE_O_A].m_data(x, y, z) += dE;  // acceptor O
                            if (m_grids[XS_TYPE_N_A].initialized())
                        ...
                        case bias_element::itype::map: {  // all or given by atom_list
                            ...
                            if (bias->atom_list.size() == 0) {  // all
                                ...
                                for (int t = 0; t < bias->atom_list.size(); ++t) {
                                    if (bias->atom_list[t] == AD_TYPE_SIZE + 1)
                                        xs_type_affected[XS_TYPE_Met_D] = true;
                                    else {
                                        ...
                                        switch (el) {
                                            ...
                                            case EL_TYPE_C: {
                                                if (ad == AD_TYPE_CG0) {
                                                    xs_type_affected[XS_TYPE_C_P_CG0] = true;
                                                    xs_type_affected[XS_TYPE_C_H_CG0] = true;
                                                } else if (ad == AD_TYPE_CG1) {
                                                    xs_type_affected[XS_TYPE_C_P_CG1] = true;
                                                    xs_type_affected[XS_TYPE_C_H_CG1] = true;
                                                } else if (ad == AD_TYPE_CG2) {
                                                    xs_type_affected[XS_TYPE_C_P_CG2] = true;
                                                    xs_type_affected[XS_TYPE_C_H_CG2] = true;
                                                } else if (ad == AD_TYPE_CG3) {
                                                    xs_type_affected[XS_TYPE_C_P_CG3] = true;
                                                    xs_type_affected[XS_TYPE_C_H_CG3] = true;
                                                } else if (ad == AD_TYPE_C) {  // add bias for all
                                                                               // kinds of Carbon
                                                    xs_type_affected[XS_TYPE_C_P_CG0] = true;
                                                    xs_type_affected[XS_TYPE_C_H_CG0] = true;
                                                    xs_type_affected[XS_TYPE_C_P_CG1] = true;
                                                    ...
                                                    xs_type_affected[XS_TYPE_C_P_CG3] = true;
                                                    xs_type_affected[XS_TYPE_C_H_CG3] = true;
                                                    xs_type_affected[XS_TYPE_C_P]
                                                        = xs_type_affected[XS_TYPE_C_H] = true;
                                                } else {
                                                    xs_type_affected[XS_TYPE_C_P]
                                                        = xs_type_affected[XS_TYPE_C_H] = true;
                                                }
                                            ...
                                            case EL_TYPE_N:
                                                xs_type_affected[XS_TYPE_N_DA]
                                                    = xs_type_affected[XS_TYPE_N_A]
                                                    = xs_type_affected[XS_TYPE_N_D]
                                                    = xs_type_affected[XS_TYPE_N_P] = true;
                                                break;
                                            case EL_TYPE_O:
                                                xs_type_affected[XS_TYPE_O_DA]
                                                    = xs_type_affected[XS_TYPE_O_A]
                                                    = xs_type_affected[XS_TYPE_O_D]
                                                    = xs_type_affected[XS_TYPE_O_P] = true;
                                                break;
                                            case EL_TYPE_S:
                                                xs_type_affected[XS_TYPE_S_P] = true;
                                                break;
                                            case EL_TYPE_P:
                                                xs_type_affected[XS_TYPE_P_P] = true;
                                                break;
                                            case EL_TYPE_F:
                                                xs_type_affected[XS_TYPE_F_H] = true;
                                                break;
                                            case EL_TYPE_Cl:
                                                xs_type_affected[XS_TYPE_Cl_H] = true;
                                                break;
                                            case EL_TYPE_Br:
                                                xs_type_affected[XS_TYPE_Br_H] = true;
                                                break;
                                            case EL_TYPE_I:
                                                xs_type_affected[XS_TYPE_I_H] = true;
                                                break;
                                            case EL_TYPE_Si:
                                                xs_type_affected[XS_TYPE_Si] = true;
                                                break;
                                            case EL_TYPE_At:
                                                xs_type_affected[XS_TYPE_At] = true;
                                                break;
                                            case EL_TYPE_Met:
                                                xs_type_affected[XS_TYPE_Met_D] = true;
                                                break;
                                            case EL_TYPE_Dummy: {
                                                if (ad == AD_TYPE_G0)
                                                    xs_type_affected[XS_TYPE_G0] = true;
                                                else if (ad == AD_TYPE_G1)
                                                    xs_type_affected[XS_TYPE_G1] = true;
                                                else if (ad == AD_TYPE_G2)
                                                    xs_type_affected[XS_TYPE_G2] = true;
                                                else if (ad == AD_TYPE_G3)
                                                    xs_type_affected[XS_TYPE_G3] = true;
                                                else if (ad == AD_TYPE_W)
                                                    xs_type_affected[XS_TYPE_SIZE]
                                                        = true;  // no W atoms in XS types
                                                else
                                                    VINA_CHECK(false);
                                                break;
                                            }
...
void cache::compute_bias(const model& m, const std::vector<bias_element> bias_list) {
    grid& g = m_grids[XS_TYPE_C_H];
