...
public:
    ...
    fl eval_fast(fl r2) const {
        ...
        sz i = sz(
            factor
            * r2);  // r2 is expected < cutoff_sqr, and cutoff_sqr * factor + 1 < n, so no overflow
        assert(i < fast.size());
        return fast[i];
    };
    pr eval_deriv(fl r2) const {
        ...
        sz i2
            = i1
              + 1;  // r2 is expected < cutoff_sqr, and cutoff_sqr * factor + 1 < n, so no overflow
...
public:
    ...
    precalculate(const ScoringFunction &sf, fl v = max_fl, fl factor = 32) {
        ...
        VINA_CHECK(sz(m_max_cutoff_sqr * m_factor) + 1
                   < m_n);  // max_cutoff_sqr * factor is the largest float we may end up converting
                            // into sz, then 1 can be added to the result
...
public:
    precalculate_byatom() {}
    precalculate_byatom(const ScoringFunction &sf, const model &model, fl v = max_fl,
                        fl factor = 32) {
        ...
        m_max_cutoff_sqr = sqr(sf.get_max_cutoff());
        m_n = sz(m_factor * m_max_cutoff_sqr)
              + 3;  // sz(factor * r^2) + 1 <= sz(factor * cutoff_sqr) +
                    // 2 <= n-1 < n  // see assert below
        ...
        VINA_CHECK(m_factor > epsilon_fl);
        VINA_CHECK(sz(m_max_cutoff_sqr * m_factor) + 1
                   < m_n);  // cutoff_sqr * factor is the largest float we may end up converting
                            // into sz, then 1 can be added to the result
        VINA_CHECK(m_max_cutoff_sqr * m_factor + 1 < m_n);
    ...
    void init_without_calculation(const ScoringFunction &sf, const model &model, fl v = max_fl,
                                  fl factor = 32) {
        // sf should not be discontinuous, even near cutoff, for the sake of the derivatives
        m_factor = factor;
        m_cutoff_sqr = sqr(sf.get_cutoff());
...
void precalculate_parallel(triangular_matrix_cuda_t *m_data_list_cpu,
                           ...
                           std::vector<model> &m_model_gpu, const flv &common_rs, const int thread);
