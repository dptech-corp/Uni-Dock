...
bool eq(
    ...
    return eq(a.R_component_1(), b.R_component_1()) && eq(a.R_component_2(), b.R_component_2())
           && eq(a.R_component_3(), b.R_component_3()) && eq(a.R_component_4(), b.R_component_4());
}
...
vec quaternion_to_angle(const qt& q) {
    ...
    if (c > -1 && c < 1) {  // c may in theory be outside [-1, 1] even with approximately normalized
                            // q, due to rounding errors
