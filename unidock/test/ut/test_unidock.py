...
class TestUniDock(ut.TestCase):
    def setUp(self):
        ...
        self.ligand = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "ligands", "1iep_ligand.pdbqt"
        )
    ...
    def test_unidock_vina(self):
        ...
        resp = subprocess.run(
            cmd, shell=True, capture_output=True, encoding="utf-8", cwd=self.workdir, check=True,
        )
        print(resp.stdout)
        ...
        score = float(
            [e for e in score_line[len("REMARK VINA RESULT:") :].split(" ") if e != ""][
                0
            ]
        )
