...
struct monte_carlo {
    ...
    monte_carlo() : max_evals(0), global_steps(2500), threads_per_ligand(2048), temperature(1.2), hunt_cap(10, 1.5, 10), min_rmsd(0.5), num_saved_mins(50), mutation_amplitude(2) {}
    ...
    void operator()(std::vector<model>& m, std::vector<output_container>& out, std::vector<precalculate_byatom>& p, triangular_matrix_cuda_t *m_data_list_gpu, const igrid& ig,
                    const vec& corner1, const vec& corner2, rng& generator, int verbosity, unsigned long long seed, std::vector<std::vector<bias_element> > &bias_batch_list) const;
