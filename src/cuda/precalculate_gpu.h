...
__device__ __forceinline__ fl sqr(fl x){
    return x * x;
}
...
__device__ __forceinline__ fl gauss_gpu(fl x, fl width) {
    return exp(-sqr(x / width));
};

__device__ __forceinline__  fl vina_gaussian_cuda_eval(sz t1, sz t2, fl r, fl cutoff, fl offset, fl width) {
    if (r >= cutoff)
        return 0.0;
    if ((t1 >= XS_TYPE_SIZE) || (t2 >= XS_TYPE_SIZE))
        return 0.0;
    return gauss_gpu(r - (optimal_distance_gpu(t1, t2) + offset), width); // hard-coded to XS atom type
};
...
__device__ __forceinline__  fl vina_hydrophobic_cuda_eval(sz t1, sz t2, fl r, fl good, fl bad, fl cutoff) {
    if (r >= cutoff)
        return 0.0;
    if ((t1 >= XS_TYPE_SIZE) || (t2 >= XS_TYPE_SIZE))
        return 0.0;
...
__device__ __forceinline__  fl vina_non_dir_h_bond_cuda_eval(sz t1, sz t2, fl r, fl good, fl bad, fl cutoff) {
    if (r >= cutoff)
        return 0.0;
    if ((t1 >= XS_TYPE_SIZE) || (t2 >= XS_TYPE_SIZE))
        return 0.0;
...
// Vinardo
__device__ __forceinline__  fl vinardo_gaussian_eval(sz t1, sz t2, fl r, fl offset, fl width, fl cutoff) {
    if (r >= cutoff)
        return 0.0;
    if ((t1 >= XS_TYPE_SIZE) || (t2 >= XS_TYPE_SIZE))
        return 0.0;
...
__device__ __forceinline__  fl vinardo_hydrophobic_eval(sz t1, sz t2, fl r, fl good, fl bad, fl cutoff) {
    if (r >= cutoff)
        return 0.0;
    if ((t1 >= XS_TYPE_SIZE) || (t2 >= XS_TYPE_SIZE))
        return 0.0;
...
__device__ __forceinline__  fl vinardo_non_dir_h_bond_eval(sz t1, sz t2, fl r, fl good, fl bad, fl cutoff) {
    if (r >= cutoff)
        return 0.0;
    if ((t1 >= XS_TYPE_SIZE) || (t2 >= XS_TYPE_SIZE))
        return 0.0;
...
// AD42
__device__ __forceinline__  fl ad4_electrostatic_eval(const fl a_charge, const fl b_charge, fl r, fl cap, fl cutoff) {
    if (r >= cutoff)
        return 0.0;
...
bool not_max_gpu(fl x) {
	return (x < 0.1 * INFINITY); /* Problem: replace max_fl with INFINITY? */
}

__device__ __forceinline__ fl ad4_solvation_eval_gpu(fl a_ad, fl a_xs, fl a_charge, fl b_ad, fl b_xs, fl b_charge, 
    fl desolvation_sigma, fl solvation_q, bool charge_dependent, fl cutoff, fl r) {
    if (r >= cutoff)
        return 0.0;
    fl q1 = a_charge;
    fl q2 = b_charge;
...
__device__ __forceinline__  fl ad4_vdw_eval(sz a_ad, sz b_ad, fl r, fl smoothing, fl cap, fl cutoff) {
    if (r >= cutoff)
        return 0.0;
    sz t1 = a_ad;
    sz t2 = b_ad;
