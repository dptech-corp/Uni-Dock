...
TEST(vina_gpu, precalculate){
    ...
    std::string ligand_name("ligands/1iep_ligand.sdf");
    std::vector<model> batch_ligands;
    batch_ligands.emplace_back(parse_ligand_from_file_no_failure(
						ligand_name, v.m_scoring_function.get_atom_typing(), false));
    gpu_batch_ligand_names.push_back(ligand_name);
    v.set_ligand_from_object_gpu(batch_ligands);
    v.global_search_gpu(1,1,1,0,5);

    Vina v_cpu("vina");
    ligand_name = "ligands/1iep_ligand.pdbqt";
