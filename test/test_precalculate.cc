...
TEST(vina_gpu, precalculate){
    ...
    //     printf("precalculated_byatom.m_data.m_data[%d]: (smooth.first, smooth.second, fast) ", i);
    //     for (int j = 0;j < 20; ++j){
    //         printf("(%f, %f, %f) ", v.m_precalculated_byatom.m_data.m_data[i].smooth[j].first,
    //         v.m_precalculated_byatom.m_data.m_data[i].smooth[j].second, v.m_precalculated_byatom_gpu[0].m_data.m_data[i].fast[j]);
    //     }
    //     printf("\n");
    // }
    for (int i = 0;i < v.m_precalculated_byatom_gpu[0].m_data.m_data.size(); ++i)
        for (int j = 0;j < v.m_precalculated_byatom_gpu[0].m_data.m_data[0].smooth.size();++j)
            EXPECT_EQ((float)v.m_precalculated_byatom_gpu[0].m_data.m_data[i].smooth[j].first, (float)v.m_precalculated_byatom.m_data.m_data[i].smooth[j].first);
